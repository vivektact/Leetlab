...
const pollBatchResults = async (tokens) => {
    while (true) {
        ...
        if (!data || !data.submissions || !Array.isArray(data.submissions)) {
            throw new Error("❌ Invalid response from Judge0: " + JSON.stringify(data));
        }
        ...
        const allDone = results.every(
            (submission) =>
                submission && submission.status && submission.status.id !== 1 && submission.status.id !== 2
        );
