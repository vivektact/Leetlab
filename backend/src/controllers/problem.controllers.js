...
const createProblem = asyncHandler(async (req, res) => {
  ...
  if (!referenceSolutions || Object.keys(referenceSolutions).length === 0) {
    return res
      .status(400)
      .json(new ApiResponse(400, null, "Reference solutions are required"))
  }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            400,
            null,
            `Language "${language}" is not supported by Judge0`
          )
        )
    }
...
const getAllProblems = asyncHandler(async (req, res) => {
  ...
  if (!problems) {
    return res
      .status(404)
      .json(new ApiResponse(404, problems, "No Problem Found"))
  }
...
const getProblemById = asyncHandler(async (req, res) => {
  ...
  if (!problem) {
    return res
      .status(404)
      .json(new ApiResponse(404, problem, "No Problem Found by id"))
  }
...
const getAllProblemsSolvedByUser = asyncHandler(async(req, res) => {
  ...
  if(!problem){
    return res.status(400).json(new ApiResponse(400, "", "No problem solved"));
  }
...
export { createProblem, getAllProblems, getProblemById, deleteProblem, getAllProblemsSolvedByUser }
